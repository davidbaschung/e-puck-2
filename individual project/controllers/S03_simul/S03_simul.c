...
// messages
#define MSG_START_1 "GOOI"
#define MSG_START_2 "GOII"
#define MSG_START_3 "GIII"
#define STOP 0
#define EXPLORER 1
#define LOVER 2
...
void robot_loop() {
    ...
    while (robot_go_on()) {
       ...
       if (strcmp(rcv, MSG_START_1) == 0 || strcmp(rcv, MSG_START_2) == 0 || strcmp(rcv, MSG_START_3) == 0) {
         counter++;
       }
       ...
       if (state == STOP) {
         ...
         set_speed(speed_left, speed_right);
         if ((prox_values[0] + prox_values[1] + prox_values[2] + prox_values[3] + prox_values[7] + prox_values[6] + prox_values[5] + prox_values[4])/8 < (0.1 * MAX_PROX)) {
...
//         if (speed_left < 0 && speed_right < 0) {
         if (prox_values[0] > 0.8*MAX_PROX && prox_values[7] > 0.8*MAX_PROX) {
           set_speed(0, 0);
           if (counter == 0) {
             send_msg(MSG_START_1);
             counter++;
           }
           else if (counter == 1) {
             send_msg(MSG_START_2);
             counter++;
           }
           else if (counter == 2) {
             send_msg(MSG_START_1);
